...
module.controller('MatriculaCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
        ...
        $scope.listarMaterias = function () {
            $http.get('./webresources/Materia', {})
                ...
                alert('Error al consultar la informaci\xf3n de materias, por favor intente m\xe1s tarde');
            });
        };
        $scope.listarMaterias();
        $scope.listarEstudiante = function () {
            $http.get('./webresources/Estudiante', {})
                    .success(function (data, status, headers, config) {
                        $scope.listaEstudiante = data;
                    }).error(function (data, status, headers, config) {
                alert('Error al consultar la informaci\xf3n de estudiante, por favor intente m\xe1s tarde');
            });
        };
        $scope.listarEstudiante();
